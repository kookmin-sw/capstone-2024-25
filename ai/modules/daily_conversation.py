...
def handle_daily_conversation(api_key, query, previous_qnas):
    # openAI api key 설정
    ...
    # 템플릿 변수 생성
    template = f"""
                    너는 질문자의 동생이야. 질문자가 외로움을 달랠 수 있도록 최대한 친절하게 대답해줘.
    ...
    # 모델 호출
    result = model.invoke(chat_prompt.format_prompt(query=query).to_messages())
